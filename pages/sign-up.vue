...
onMounted(async () => {
  ...
  // Configure redirection for sign-in links
  setTimeout(() => {
...
<template>
  <div v-if="!showAuthTransition" class="signup-container">
    <div class="signup-card">
      <div class="signup-header">
        <h1 class="signup-title">Create Account</h1>
        <p class="signup-subtitle">Sign up to get started with our platform.</p>
      </div>

      <!-- Spinner overlay that displays over all content during loading -->
      <div v-if="showSpinner" class="spinner-overlay">
        <div class="spinner"></div>
        <p class="loading-text">Loading sign-up form...</p>
      </div>

      <!-- Signup form (will be visually hidden by spinner until loaded) -->
      <client-only>
        <SignUp
          v-if="isMounted"
          :sign-in-url="'/login'"
